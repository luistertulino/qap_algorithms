...
void print_output()
{
  ...
  for (int k=1; k < n_k; k++)
  {
    ss << std::fixed << std::setprecision(2) << corr[k] << "-";
  }
  ss << std::fixed << std::setprecision(1) << overlap << "-" << num_inst;
  std::string file = "reallike/Ter-rl-" + ss.str() + ".in";
  f = fopen(file.c_str(), "a+");
  fprintf(f, "%i %i\n", n_fac, n_k);

  printf("facilities = %d objectives = %d max_distances = %d max flows = %d overlap =  %.2f seed = %ld\n", n_fac, n_k, max_dist, max_flow, overlap, init_seed);
  for(k=1;k<n_k;k++)
    {
